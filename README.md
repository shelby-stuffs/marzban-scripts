...
- **Install Marzban with MariaDB and Dev branch**:
  ...
  ```bash
  sudo bash -c "$(curl -sL https://github.com/shelby-stuffs/Marzban-scripts/raw/master/marzban.sh)" @ install --database mariadb --dev
  ```

- **Install Marzban with MariaDB and Manual version**:

  ```bash
  sudo bash -c "$(curl -sL https://github.com/shelby-stuffs/Marzban-scripts/raw/master/marzban.sh)" @ install --database mariadb --version v0.5.2
  ```
...
```bash
sudo bash -c "$(curl -sL https://github.com/shelby-stuffs/Marzban-scripts/raw/master/marzban-node.sh)" @ install
```
Install Marzban-node on your server using this command with custom name:
```bash
sudo bash -c "$(curl -sL https://github.com/shelby-stuffs/Marzban-scripts/raw/master/marzban-node.sh)" @ install --name marzban-node2
```
Or you can only install this script (marzban-node command) on your server by using this command
```bash
sudo bash -c "$(curl -sL https://github.com/shelby-stuffs/Marzban-scripts/raw/master/marzban-node.sh)" @ install-script
```
